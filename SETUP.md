...
Steps 3 to 6 cover the per-lead path. For the public page, repeat 3 and 4 against the
`agoutbound` client with **Public ticked**, then open `/book/agoutbound/<slug>` in a private
window and book as a stranger would.
...
**Per-lead links** (`/b/<token>`) are minted by `POST /api/links`, single-use, and carry
`lead_id`, `campaign_id`, `wave` and step invisibly into the booking. This is the path that
gives you attribution, and it is `noindex`.

**The public page** (`/book/<client>/<event>`) has no token. Anyone with the URL can book,
the visitor types their own name and email, and the booking carries no campaign attribution
— correctly, because a website visitor has no campaign. It is indexable.

An event type is only reachable publicly when `is_public` is true, which defaults to false.
Every event type you build for outreach therefore stays private unless you deliberately open
it. A private page and a non-existent one return the same 404, so nobody can discover your
clients' event types by guessing slugs.

Public bookings are limited to one future booking per email per event type and three per
hour per address, with a honeypot field. Those limits are the only gate on that page.

To put it on your own domain, point `agoutbound.co.uk/book` at
`/book/agoutbound/<event-slug>`.
