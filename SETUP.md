...
**One secret currently does two jobs.** `LINKS_API_KEY` is both the bearer token for
`POST /api/links` *and* the HMAC key that signs connect invites, OAuth state, manage tokens
and operator sessions. So anyone you hand the minting key to — a script, a teammate, a
future automation — can also forge a manage token for any booking and sign themselves an
operator session. That is fine while you are the only holder. It stops being fine the first
time it leaves your machine. Splitting it into `LINKS_API_KEY` and a separate `SIGNING_KEY`
is a small change and worth doing before anyone else gets a copy.
...
```bash
node -e "console.log('LINKS_API_KEY=' + require('crypto').randomBytes(32).toString('base64url')); console.log('CRON_SECRET=' + require('crypto').randomBytes(32).toString('base64url'))"
```
...
| `LINKS_API_KEY` | yes | Minting and every signed token fail |
